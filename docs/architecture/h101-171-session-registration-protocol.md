...
**H101-178 revision (2026-09-22):** incorporates Angela’s H101-177 product rulings, relayed in dispatch `2026-09-22T10-28-27-008Z-ae3b54`: cooperative check-in is the first registration product; a descriptive participation profile replaces the original execution-profile prerequisite. These product decisions do not authorize construction. H101-174 delivery scope was subsequently settled below.

## Owner intent and attachment-first delivery
...
**H101-174 owner decision (2026-09-22): attachment first.** Relayed in dispatch `2026-09-22T11-57-29-613Z-892a55`, the owner chose option A:

> the A can be the one to keep to have it simpler. but have B on the backlog for the near future.

The current delivery scope is registration of an already-open session. Managed-launch delivery and its outstanding evidence are **deferred, not cancelled**. Both attachment and managed launch remain a near-future backlog direction; this records priority intent, not a deadline or authorization to build both now. Preserve existing managed-start work and the shared participation/coexistence design below rather than invent a second protocol or remove implemented capabilities.

The managed-launch evidence gap remains explicit. [Item 1 D13 and Layer B](../quality/phase3-exit-criteria.md) require managed start; external registration cannot satisfy them by writing a manifest. [Kelly’s H101-172 ruling](../quality/h101-172-layerb-registration-ruling.md) keeps registration outside item 1 Layer B and recommends separate registration acceptance. Kelly owns the gate disposition for the owner's deferral. This scope amendment neither marks managed-launch evidence satisfied nor silently rewrites any gate. Neither entry mode's evidence proves the other.
...
**Attachment-first admission rule (H101-174):** one agent cannot acquire a new external binding while it has unresolved managed execution, and cannot acquire a new managed execution while its external attachment slot is occupied. This is an enforced core admission rule for the first delivery, not deferred coexistence work.

- Reject invitation issuance and pairing redemption with `Conflict` for the same agent when a managed run is `Starting`, `Running`, `Stopping` or `RecoveryRequired`. Recheck at redemption; an invitation is not a reservation or permission to bypass a later conflict. There is **no managed-bootstrap exception in attachment-first scope**. Historical execution records whose execution is already resolved do not block merely because they exist; registration itself cannot resolve them.
- Reject a new managed start for an agent whose external registration is Connected, Stale or Disconnected. These states all retain the attachment slot. Apply the check to existing trusted CLI and other managed-start entry points as well as the new registration interface; hiding launch from the attachment UI is insufficient. Closed registrations no longer occupy the slot, but all ordinary launch approval and run-exclusion rules still apply.
- Serialize pairing admission and managed-start admission against the same authoritative state. Concurrent requests cannot both acquire the agent. The losing request returns `Conflict` without a new binding or start intent and without dispatching a child. Preserve normal same-request receipt recovery: replay of an already committed operation is not a new admission or permission for another spawn.
- Staleness, host restart, managed-launch deferral, matching profile labels and possession of an invitation never clear either exclusion. Only the existing explicit external unregister/user replacement rules release or replace that binding; neither action clears managed-run uncertainty. Managed recovery resolution remains subject to its separate security boundary. This conservative blocking can cost availability; timeout-based takeover is not an acceptable shortcut.

These checks govern supported product entry points, not independently launched same-user programs or direct file tampering. They add no hostile same-user isolation claim. Security review must assess the actual legacy entry-point coverage, and QA must cover both conflict directions, the admission race, stale/disconnected slots, replay, and explicit closure without managed-run clearance.

**Deferred coexistence design (H101-198):** retain the shared participation contract and prospective exact-run managed bootstrap as backlog design, not an available exception. Reintroducing that bootstrap requires an explicit scope decision, binding/admission specification, security review and tests. Managed launch is deferred, not cancelled; this first-delivery exclusion prevents an accidental takeover while both-path delivery remains on the near-future backlog.
...
**Remaining review boundaries:** H101-174 is settled: attachment-first delivery, managed launch deferred with its evidence gap retained, and both paths on the near-future backlog. Cooperative check-in and the lightweight participation profile are decided by H101-177. Do not reopen these settled product decisions as prerequisites. Security still reviews pairing authority and residual exposure before construction. Kelly’s [H101-172](../quality/h101-172-layerb-registration-ruling.md) keeps registration evidence separate from managed-start Layer B; neither the profile split nor cooperative check-in changes that ruling. No code, gate changes, tool launches, credentials or provider calls were made for this specification or its H101-178 and H101-174 revisions.
